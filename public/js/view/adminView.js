...
async function renderVacancy(tempStorageCars = null, noupdate = false) {
  ...
  if (noupdate === true) {
    ...
    parkingLotVacancy.innerHTML = `${
      parkingLotSpaceInt - (await tempStorageCars.length)
    } Vacancy`;
  }
}
...
function renderCarCard(event, carList = tempStorageCars) {
  if (event.target.classList.contains("list-group-item")) {
    ...
    let parkingLotFee = document.querySelector("#parkingLotFee");
    let parkingLotFeeInt = parseInt(
      parkingLotFee.innerText.split(" ")[1].split("/")[0]
    );

    let [hours, minutes, subTotal] = pastTimetoFee(
      new Date(queryResult[0].enter_time),
      new Date(queryResult[0].green_light),
      parkingLotFeeInt
    );
