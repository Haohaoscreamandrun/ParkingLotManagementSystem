...
async function renderCarDetails(carList, lotList) {
  ...
  if (new Date(car.green_light) - new Date() > 0) {
    ...
    new Date(car.green_light) - new Date() < 0 &&
    new Date(car.green_light) - new Date(car.enter_time) > 0
  ) {
    // logic of overtime
    ...
    paidWarning.children[1].innerText =
      "Please pay for additional parking fee since you haven't left in 15 minutes.";
  } else {
    paidCheck.checked = false;
