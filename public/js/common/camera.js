...
function checkUploadCondition() {
  let allSame = arrayLicense.every(
    (val) => val === arrayLicense[0] && val !== ""
  );
  let averageConfidence =
    arrayConfidence.reduce((sum, val) => sum + val, 0) / arrayConfidence.length;
...
async function processRecognition() {
  ...
  if (arrayLicense.length === 3 && checkUploadCondition()) {
    ...
    // show on screen
    recognizedPlateInput.value = `Successful! ${license}, conf. lv: ${
      arrayConfidence.reduce((sum, val) => sum + val, 0) /
      arrayConfidence.length
    }`;
