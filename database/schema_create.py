...
mydb = mysql.connector.connect(
    host=host_RDS, user=user_admin, password=password_admin, database="parkinglot"
)
...
## taipei data
with open(
    "ParkingLotProject/database/taipei_city_parking_lot.json", encoding="utf-8"
) as file:
    taipei_parking_lots = json.load(file)

## new taipei data
with open(
    "ParkingLotProject/database/new_taipei_city_parking_lot.json", encoding="utf-8"
) as file:
    new_taipei_parking_lots = json.load(file)

# taoyuan data
with open(
    "ParkingLotProject/database/taoyuan_city_parking_lot.json", encoding="utf-8"
) as file:
    taoyuan_parking_lots = json.load(file)

# hsinchu data
with open(
    "ParkingLotProject/database/hsinchu_city_parking_lot.json", encoding="utf-8"
) as file:
...
if len(myresult) != 0:
    ...
    def commit_db(name, latitude, longitude, address, total_space, parking_fee, index):
        sql = """
        ...
        try:
            if not skip_this:
                ...
                val = (name, latitude, longitude, address, total_space, parking_fee, 2)
                mycursor.execute(sql, val)
    ...
    for index, data in enumerate(taipei_parking_lots["features"]):
        ...
        total_space = (
            int(data["properties"]["num_s"])
            if data["properties"]["num_s"] is not None
            else None
        )
        parking_fee = extract_first_integer(data["properties"]["feeb"])

        commit_db(name, latitude, longitude, address, total_space, parking_fee, index)
    mydb.commit()

    # loop new taipei data
    for index, data in enumerate(new_taipei_parking_lots):
        name = data["NAME"]
        longitude, latitude = transformer.transform(data["TW97X"], data["TW97Y"])
    ...
    for index, data in enumerate(taoyuan_parking_lots["parkingLots"]):
        ...
        address = data["address"]
        total_space = (
            int(data["totalSpace"]) if data["totalSpace"] is not None else None
        )
