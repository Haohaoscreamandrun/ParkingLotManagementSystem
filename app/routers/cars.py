...
from ..model.commit import car_enter, car_exit, car_update, grant_green_light
from ..model.execute import (
    cars_in_parking_lot,
    car_by_carID,
    vacancy_lookup,
    car_by_license,
    admin_parking_lot_lookup,
)
from ..model.s3 import copy_file, delete_file
from ..dependencies import convert_timezone, admin_validation_dependency
...
@router.get(
    ...
    responses={
        200: {
            "model": ReturnCarsObj,
            "description": "Successful on retrieve all cars information from specific parking lot, return none if there's no car in that parking lot.",
        }
    },
...
@router.get(
    ...
    responses={
        200: {
            "model": ReturnCarsObj,
            "description": "Successful on retrieve specific car information, return none if there's no car by that id.",
        }
    },
    response_class=JSONResponse,
    response_model=ReturnCarsObj,
...
@router.post(
    ...
    responses={
        ...
        400: {
            "model": Error,
            "description": "Failed to post a new car due to no vacancy or doubled license.",
        },
    },
    ...
    try:
        ...
        if vacancy > 0 and len(double) == 0:
            ...
            return JSONResponse(
                ...
                content={"error": True, "message": "There's no vacancy right now!"},
            )
        elif len(double) > 0:
            return JSONResponse(
                status_code=status.HTTP_400_BAD_REQUEST,
...
@router.put(
    ...
    responses={
        200: {
            "model": Success,
            "description": "Successful on modify specific car information",
        },
        400: {
            "model": Error,
            "description": "Unauthorized personal/admin to update car information",
        },
    },
    response_class=JSONResponse,
    summary="The API to modify specific car information based on posted information.",
)
async def put_car_by_id(
    admin: Annotated[dict, Depends(admin_validation_dependency)], updates: PutCarInfo
):
...
@router.delete(
    ...
    responses={
        200: {
            "model": Success,
            "description": "Successful on delete specific car information",
        },
...
async def delete_car_by_license(data: DeleteCarInfo):
    ...
    try:
        ...
        if car_green_light > now:
            ...
            return JSONResponse(
                status_code=status.HTTP_400_BAD_REQUEST,
                content={"error": True, "message": "Not yet paid."},
            )
        elif car_green_light != car_enter_time and car_green_light < now:
            return JSONResponse(
                status_code=status.HTTP_400_BAD_REQUEST,
                content={"error": True, "message": "Exit overtime."},
            )
    except (HTTPException, StarletteHTTPException) as exc:
        raise HTTPException(status_code=exc.status_code, detail=exc.detail)
