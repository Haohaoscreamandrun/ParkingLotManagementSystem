...
from ..config.basemodel import Error, RetrunParkingLotObj
from ..model.execute import parkinglot_by_id, parking_lot_by_location
...
@router.get(
    ...
    responses={
        200: {
            "model": RetrunParkingLotObj,
            "description": "Successful on retrieve parking lots information based on distance or specific parking lot information, or else return noting if condition is not met.",
        }
    },
...
async def get_parking_lots(
    ...
    number: int = Query(
        15, description="Number of parking lots to retrieve if location is provided"
    ),
    lot_id: str = Query(
        None, alias="lotID", description="Parking lot ID for specific lot retrieval"
    ),
):
    try:
        ...
        # Cond 2: latitude and longitude is provided
            ...
            # If no parking lots found, return the nearest 15
            if len(my_result) == 0:
